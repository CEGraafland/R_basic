...
for(j in c(1,2,3,4,5)){
  ...
  filestabu1 <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/mmhc", full.names = T, pattern = pattern)
  filestabu1names <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/mmhc", pattern = pattern)
  filestabu1names <- gsub(".rda", "", filestabu1names)
  filestabu1names
...
##########################################################################
for(j in c(1,2,3,4,5)){
  pattern <- paste0("h2pc_",j,"_eBIC_g")
  filestabu1 <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/h2pc", full.names = T, pattern = pattern)
  filestabu1names <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/h2pc", pattern = pattern)
  filestabu1names <- gsub(".rda", "", filestabu1names)
  filestabu1names
  ...
  names(variablelisttabu1$networks) <- names
  assign(paste0("h2pc_",j,"_eBIC"),variablelisttabu1)
}
...
for(j in c(1,2,3,4,5)){
  ...
  filestabu1 <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/pcstable", full.names = T, pattern = pattern)
...
for(j in c(1,2,3,4,5)){
  pattern <- paste0("pcstable2_withnet_",j)
  filestabu1 <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/pcstable", full.names = T, pattern = pattern)
  # filestabu1names <- list.files("/run/user/1000/gvfs/sftp:host=ui01.macc.unican.es/vols/seal/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/pcstable", pattern = pattern)
  # filestabu1names <- gsub(".rda", "", filestabu1names)
  # filestabu1names
...
for(j in c(1,2,3,4,5)){
  ...
  filestabu1 <- list.files("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/gs", full.names = T, pattern = pattern)
...
ejes_gamma_simple_vstructs <- function(nets, moral = FALSE){
  networkdata <- nets$networkdata
  network <- nets$networks
  xeje <- (nets$networkdata$gamma)*2
  miss <- !is.na(xeje)
  vs <- lapply(network,vstructssimpprop, moral = moral)
  yeje <- sapply(vs, function(x) x[2])
...
sum(log10(mmhc_1_eBIC_g0.1$networkdata$tests + mmhc_1_eBIC_g0.1$begin$learning$ntests),
...
log10(mmhc_5_eBIC_g0.1$networkdata$tests + mmhc_5_eBIC_g0.1$begin$learning$ntests))/5

sum(log10(mmhc_1_eBIC_g0$networkdata$tests + mmhc_1_eBIC_g0$begin$learning$ntests),
...
log10(mmhc_5_eBIC_g0$networkdata$tests + mmhc_5_eBIC_g0$begin$learning$ntests))/5

sum(log10(mmhc_1_eBIC_g0.25$networkdata$tests + mmhc_1_eBIC_g0.25$begin$learning$ntests),
...
log10(mmhc_5_eBIC_g0.25$networkdata$tests + mmhc_5_eBIC_g0.25$begin$learning$ntests))/5
...
plotname <- paste0("/run/user/1000/gvfs/sftp:host=ui01.macc.unican.es/vols/seal/oceano/gmeteo/WORK/lisette/Trabajo/Writing/paper/figures/testsvsgammaQuad3.pdf")
plotname <- paste0("/run/user/1000/gvfs/sftp:host=ui01.macc.unican.es/vols/seal/oceano/gmeteo/WORK/lisette/Trabajo/Writing/paper/figures/testsvsgammaQuad4.pdf")
...
ejespcstable <- lapply(list(pcstable2_withnet_1, pcstable2_withnet_2, pcstable2_withnet_3,pcstable2_withnet_4,pcstable2_withnet_5),ejes_gamma_tests)
ejesgs <- lapply(list(gs_withnet_1, gs_withnet_2, gs_withnet_3, gs_withnet_4, gs_withnet_5), ejes_gamma_tests)
...
# medias.
# TAbu
binds<- rbind(tabu_1_eBIC$networkdata$tests,tabu_2_eBIC$networkdata$tests,tabu_3_eBIC$networkdata$tests,tabu_4_eBIC$networkdata$tests,tabu_5_eBIC$networkdata$tests)
yejemed <- log10(colMeans(binds))
tabu_med_eBIC <- list(xeje = tabu_1_eBIC$networkdata$gamma, yeje  = yejemed)
# HC
bindsHC<- rbind(hc_1_eBIC$networkdata$tests,hc_2_eBIC$networkdata$tests,hc_3_eBIC$networkdata$tests,hc_4_eBIC$networkdata$tests,hc_5_eBIC$networkdata$tests)
yejemedHC <- log10(colMeans(bindsHC))
hc_med_eBIC <- list(xeje = hc_1_eBIC$networkdata$gamma, yejemedHC  = yejemedHC)
# mmhc
bindsmmhc<- rbind(ejesmmhcBIC[[1]][[2]],ejesmmhcBIC[[2]][[2]],ejesmmhcBIC[[3]][[2]],ejesmmhcBIC[[4]][[2]],ejesmmhcBIC[[5]][[2]])
yejemedmmHC <- colMeans(bindsmmhc)
mmhc_med_eBIC <- list(xeje = mmhc_1_eBIC$networkdata$gamma, yeje  = yejemedmmHC)
# h2pc
bindsh2pc<- rbind(ejesh2pcBIC[[1]][[2]],ejesh2pcBIC[[2]][[2]],ejesh2pcBIC[[3]][[2]],ejesh2pcBIC[[4]][[2]],ejesh2pcBIC[[5]][[2]])
yejemedh2pc <- colMeans(bindsh2pc)
h2pc_med_eBIC <- list(xeje = h2pc_1_eBIC$networkdata$gamma, yeje  = yejemedh2pc)
# pc
bindspc<- rbind(ejespcstable[[1]][[2]],ejespcstable[[2]][[2]],ejespcstable[[3]][[2]],ejespcstable[[4]][[2]],ejespcstable[[5]][[2]])
yejemedpc <- colMeans(bindspc)
pc_med_eBIC <- list(xeje = pcstable2_withnet_1$networkdata$gammas, yeje  = yejemedpc)
pc_med_eBIC <- list(xeje = pcstable2_1$networkdata$gammas, yeje  = yejemedpc)
# gs
bindsgs<- rbind(ejesgs[[1]][[2]],ejesgs[[2]][[2]],ejesgs[[3]][[2]],ejesgs[[4]][[2]],ejesgs[[5]][[2]])
yejemedgs <- colMeans(bindsgs)
gs_med_eBIC <- list(xeje = gs_withnet_1$networkdata$gammas, yeje  = yejemedgs)
gs_med_eBIC <- list(xeje = gs_1$networkdata$gammas, yeje  = yejemedgs)
###############################################################################

plot(ejestabuBIC[[1]][[1]], ejestabuBIC[[1]][[2]], type = "p",
     col = rgb(255,0,0, alpha = 255, maxColorValue = 255),
     main = "(a) Speed",
...
points(x = tabu_med_eBIC$xeje*2, y = tabu_med_eBIC$yeje, pch = 23, bg = rgb(255,0,0, alpha = 255, maxColorValue = 255))
...
points(hc_med_eBIC$xeje*2,hc_med_eBIC$yeje, pch = 23, bg = rgb(255,165,0, alpha = 255, maxColorValue = 255))
...
points(mmhc_med_eBIC$xeje*2, mmhc_med_eBIC$yeje, pch = 23, bg = rgb(0,255,0, alpha = 255, maxColorValue = 255))

points(ejesh2pcBIC[[1]][[1]], ejesh2pcBIC[[1]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[2]][[1]], ejesh2pcBIC[[2]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[3]][[1]], ejesh2pcBIC[[3]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[4]][[1]], ejesh2pcBIC[[4]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[5]][[1]], ejesh2pcBIC[[5]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(h2pc_med_eBIC$xeje*2, h2pc_med_eBIC$yeje, pch = 23, bg = rgb(0,100,0, alpha = 255, maxColorValue = 255))
...
points(pc_med_eBIC$xeje*2, pc_med_eBIC$yeje, pch = 23, bg = rgb(0,0,128, alpha = 125, maxColorValue = 255))
...
points(gs_med_eBIC$xeje*2, gs_med_eBIC$yeje, pch = 23, bg = rgb(0,0,255, alpha = 255, maxColorValue = 255))
...
points(x = c(0.2), y = c(7.12), pch = 23, bg = rgb(0,100,0, alpha = 255, maxColorValue = 255))
...
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC, h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_log)
ejespcstable <- lapply(list(pcstable2_1, pcstable2_2, pcstable2_3,pcstable2_4,pcstable2_5),ejes_gamma_log)
ejespcstable <- lapply(list(pcstable2_withnet_1, pcstable2_withnet_2, pcstable2_3,pcstable2_4,pcstable2_5),ejes_gamma_log)
...
points(ejesgs[[5]][[1]], ejesgs[[5]][[2]], col = rgb(0,0,255, alpha = 255, maxColorValue = 255))
points(ejesgs[[2]][[1]], ejesgs[[2]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[3]][[1]], ejesgs[[3]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[4]][[1]], ejesgs[[4]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[1]][[1]], ejesgs[[1]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
...
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC,h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_edges)
...
points(x = c(0.2), y = c(2600),col = rgb(255,0,0, alpha = 255, maxColorValue = 255))
points(x = c(0), y = c(2650),col = rgb(255,0,0, alpha = 255, maxColorValue = 255))
...
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC, h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_size)
...
points(ejesh2pcBIC[[5]][[1]], ejesh2pcBIC[[5]][[2]], pch = 23, bg = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[2]][[1]], ejesh2pcBIC[[2]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[3]][[1]], ejesh2pcBIC[[3]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[4]][[1]], ejesh2pcBIC[[4]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[1]][[1]], ejesh2pcBIC[[1]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
...
#############################################################################################
# gamma plot number of dsep vstructs
##############################################################################################
ejestabuBIC <- lapply(list(tabu_1_eBIC,tabu_2_eBIC,tabu_3_eBIC,tabu_4_eBIC,tabu_5_eBIC),ejes_gamma_vstructs)
ejeshcBIC <- lapply(list(hc_1_eBIC, hc_2_eBIC, hc_3_eBIC,hc_4_eBIC,hc_5_eBIC),ejes_gamma_vstructs)
ejesmmhcBIC <- lapply(list(mmhc_1_eBIC,mmhc_2_eBIC, mmhc_3_eBIC,mmhc_4_eBIC,mmhc_5_eBIC), ejes_gamma_vstructs)
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC, h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_vstructs)
ejespcstable <- lapply(list(pcstable2_withnet_1,pcstable2_withnet_2),ejes_gamma_vstructs)
ejesgs <- lapply(list(gs_withnet_1,gs_withnet_2,gs_withnet_3,gs_withnet_4,gs_withnet_5), ejes_gamma_vstructs)


plot(ejespcstable[[1]][[1]], ejespcstable[[1]][[2]], type = "p",
     #lwd = 1,
     pch = 23, col = rgb(0,0,128, alpha = 125, maxColorValue = 255),
     main = "number of d-seperating vstructures without evidence",
     xlab = expression(paste(gamma)), 
     ylab = expression(paste("nvstructs")),
     # xlim = c(0,1),
     ylim = c(0,1)
)
...
#############################################################################################
# gamma plot number of simple immoral vstructs in equivalence class
##############################################################################################
whichvstructs <- 'moral' 
if(whichvstructs == "immoral"){
ejestabuBIC <- lapply(list(tabu_1_eBIC,tabu_2_eBIC,tabu_3_eBIC,tabu_4_eBIC,tabu_5_eBIC),ejes_gamma_simple_vstructs, moral = FALSE)
ejeshcBIC <- lapply(list(hc_1_eBIC, hc_2_eBIC, hc_3_eBIC,hc_4_eBIC,hc_5_eBIC),ejes_gamma_simple_vstructs, moral = FALSE)
ejesmmhcBIC <- lapply(list(mmhc_1_eBIC,mmhc_2_eBIC, mmhc_3_eBIC,mmhc_4_eBIC,mmhc_5_eBIC), ejes_gamma_simple_vstructs, moral = FALSE)
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC, h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_simple_vstructs, moral = FALSE)
ejespcstable <- lapply(list(pcstable2_withnet_1,pcstable2_withnet_2,pcstable2_withnet_3,pcstable2_withnet_4,pcstable2_withnet_5),ejes_gamma_simple_vstructs, moral = FALSE)
ejesgs <- lapply(list(gs_withnet_1,gs_withnet_2,gs_withnet_3,gs_withnet_4,gs_withnet_5), ejes_gamma_simple_vstructs, moral = FALSE)
} else if (whichvstructs == "moral"){
ejestabuBIC <- lapply(list(tabu_1_eBIC,tabu_2_eBIC,tabu_3_eBIC,tabu_4_eBIC,tabu_5_eBIC),ejes_gamma_simple_vstructs, moral = TRUE)
ejeshcBIC <- lapply(list(hc_1_eBIC, hc_2_eBIC, hc_3_eBIC,hc_4_eBIC,hc_5_eBIC),ejes_gamma_simple_vstructs, moral = TRUE)
ejesmmhcBIC <- lapply(list(mmhc_1_eBIC,mmhc_2_eBIC, mmhc_3_eBIC,mmhc_4_eBIC,mmhc_5_eBIC), ejes_gamma_simple_vstructs, moral = TRUE)
ejesh2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC, h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_simple_vstructs, moral = TRUE)
ejespcstable <- lapply(list(pcstable2_withnet_1,pcstable2_withnet_2,pcstable2_withnet_3,pcstable2_withnet_4,pcstable2_withnet_5),ejes_gamma_simple_vstructs, moral = TRUE)
ejesgs <- lapply(list(gs_withnet_1,gs_withnet_2,gs_withnet_3,gs_withnet_4,gs_withnet_5), ejes_gamma_simple_vstructs, moral = TRUE)
}



ejespcstable_min <- lapply(list(pcstable2_1, pcstable2_2, pcstable2_3,pcstable2_4,pcstable2_5),ejes_gamma_edges)
ejesgs_min <- lapply(list(gs_1, gs_2, gs_3, gs_4, gs_5), ejes_gamma_edges)

pcindex <- list()
for (i in 1:5){
x1 <- ejespcstable_min[[i]][[1]]
x2 <- ejespcstable[[i]][[1]] 
pcindex[[i]] <- !x2 %in% setdiff(x2,x1)
}
...
for (i in 1:5){
  x1 <- ejesgs_min[[i]][[1]]
  x2 <- ejesgs[[i]][[1]] 
  gsindex[[i]] <- !x2 %in% setdiff(x2,x1)
}
gsindex

plot(ejespcstable[[1]][[1]][pcindex[[1]]], ejespcstable[[1]][[2]][pcindex[[1]]], type = "p",
     #lwd = 1,
     col = rgb(0,0,128, alpha = 125, maxColorValue = 255),
     main = paste0("(d) Relative number of ",whichvstructs," v-structures"),
     xlab = expression(paste(gamma)), 
     ylab = expression(paste("|v-structures/|number of 2-paths|")),
     xlim = c(0,20),
     ylim = c(0,0.55)
)
...
col2rgb("navyblue")
points(ejespcstable[[5]][[1]][pcindex[[5]]], ejespcstable[[5]][[2]][pcindex[[5]]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[2]][[1]][pcindex[[4]]], ejespcstable[[2]][[2]][pcindex[[4]]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[3]][[1]][pcindex[[3]]], ejespcstable[[3]][[2]][pcindex[[3]]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[4]][[1]][pcindex[[2]]], ejespcstable[[4]][[2]][pcindex[[2]]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))


points(ejespcstable[[5]][[1]][!pcindex[[5]]], ejespcstable[[5]][[2]][!pcindex[[5]]], pch = 16, col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[2]][[1]][!pcindex[[4]]], ejespcstable[[2]][[2]][!pcindex[[4]]], pch = 16, col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[3]][[1]][!pcindex[[3]]], ejespcstable[[3]][[2]][!pcindex[[3]]], pch = 16, col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[4]][[1]][!pcindex[[2]]], ejespcstable[[4]][[2]][!pcindex[[2]]], pch = 16, col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[1]][[1]][!pcindex[[1]]], ejespcstable[[1]][[2]][!pcindex[[1]]], pch = 16, col = rgb(0,0,128, alpha = 125, maxColorValue = 255))

points(ejesgs[[5]][[1]][gsindex[[5]]], ejesgs[[5]][[2]][gsindex[[5]]], col = rgb(0,0,255, alpha = 255, maxColorValue = 255))
points(ejesgs[[2]][[1]][gsindex[[2]]], ejesgs[[2]][[2]][gsindex[[2]]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[3]][[1]][gsindex[[3]]], ejesgs[[3]][[2]][gsindex[[3]]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[4]][[1]][gsindex[[4]]], ejesgs[[4]][[2]][gsindex[[4]]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[1]][[1]][gsindex[[1]]], ejesgs[[1]][[2]][gsindex[[1]]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))

points(ejesgs[[5]][[1]][!gsindex[[5]]], ejesgs[[5]][[2]][!gsindex[[5]]], pch = 16,col = rgb(0,0,255, alpha = 255, maxColorValue = 255))
points(ejesgs[[2]][[1]][!gsindex[[2]]], ejesgs[[2]][[2]][!gsindex[[2]]], pch = 16,col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[3]][[1]][!gsindex[[3]]], ejesgs[[3]][[2]][!gsindex[[3]]], pch = 16,col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[4]][[1]][!gsindex[[4]]], ejesgs[[4]][[2]][!gsindex[[4]]], pch = 16,col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[1]][[1]][!gsindex[[1]]], ejesgs[[1]][[2]][!gsindex[[1]]], pch = 16,col = rgb(0,0,255, alpha = 125, maxColorValue = 255))

vstructs(gs_withnet_1$networks$gs_10d_g1, moral = TRUE)
vstructs(gs_withnet_1$networks$gs_10d_g1, moral = FALSE)
plot.vstruct.bn(vstructs(gs_withnet_1$networks$gs_10d_g1, moral = TRUE), data.network = TimeCoordsAnom_from_Grid_rms(tas_ncep_10d, rms = TRUE))
pcsta
plot(ejeshcBIC[[1]][[1]],ejes_gamma_simple_vstructs(hc_1_eBIC, moral = TRUE)[[2]]-ejes_gamma_simple_vstructs(hc_1_eBIC, moral = FALSE)[[2]])
plot(ejespcstable[[1]][[1]],ejes_gamma_simple_vstructs(pcstable2_withnet_1, moral = TRUE)[[2]]-ejes_gamma_simple_vstructs(pcstable2_withnet_1, moral = FALSE)[[2]])
plot(ejesgs[[1]][[1]],ejes_gamma_simple_vstructs(gs_withnet_1, moral = TRUE)[[2]]-ejes_gamma_simple_vstructs(gs_withnet_1, moral = FALSE)[[2]])
plot(ejeshcBIC[[1]][[1]],ejes_gamma_simple_vstructs(hc_1_eBIC, moral = FALSE)[[2]])
###########################################################################################
# For CONSTRAINED based: relative amount of setted arcs after phase 1. 
###########################################################################################
pccpdags <- lapply(pcstable2_withnet_1$networks, cpdag, moral = FALSE)
pcstable2_withnet_1$networks
nundirarcs0 <- sapply(pccpdags,narcs)
dirarcs0 <- lapply(pccpdags,directed.arcs)
ndirarcs0 <- sapply(dirarcs0,nrow)

plot(nundirarcs0, ndirarcs0/nundirarcs0, 
     pch = 23, bg = rgb(0,0,128, alpha = 125, maxColorValue = 255),
     main = "Directed arcs in equivalence class",
     xlab = "|E|",
     ylab = "|Edir| / |E|",
     ylim = c(0,1)
)
...
# df <- pcstable2_withnet_1$networkdata
# plot(df$edgeseqclass,((df$edgeseqclass-df$undeqclass)/df$edgeseqclass), 
#      pch = 23, bg = rgb(0,0,128, alpha = 125, maxColorValue = 255),
#      main = "Directed arcs in equivalence class",
#      xlab = "|E|",
#      ylab = "|Edir| / |E|",
#      ylim = c(0,1)
#      )
# df2 <- gs_withnet_1$networkdata
# points(df2$edgeseqclass, ((df2$edgeseqclass-df2$undeqclass)/df2$edgeseqclass))

hccpdags <- lapply(hc_1_eBIC$networks,cpdag, moral = FALSE)
nundirarcs <- sapply(hccpdags,narcs)
dirarcs <- lapply(hccpdags,directed.arcs)
ndirarcs <- sapply(dirarcs,nrow)

points(nundirarcs, ndirarcs/nundirarcs, col = rgb(255,165,0, alpha = 255, maxColorValue = 255))


h2pccpdags <- lapply(h2pc_1_eBIC$networks,cpdag, moral = FALSE)
nundirarcs2 <- sapply(h2pccpdags,narcs)
dirarcs2 <- lapply(h2pccpdags,directed.arcs)
ndirarcs2 <- sapply(dirarcs2,nrow)

points(nundirarcs2, ndirarcs2/nundirarcs2, col = rgb(0,100,0, alpha = 255, maxColorValue = 255))

gscpdags <- lapply(gs_withnet_1$networks,cpdag, moral = FALSE)
nundirarcs3 <- sapply(gscpdags,narcs)
dirarcs3 <- lapply(gscpdags,directed.arcs)
ndirarcs3 <- sapply(dirarcs3,nrow)

points(nundirarcs3, ndirarcs3/nundirarcs3, col = rgb(0,255,0, alpha = 255, maxColorValue = 255))

ejestabuBIC_vstruct_1

gs_withnet_5$networkdata
##############################################################################################
# plot mean
##############################################################################################
...
#######################################################################################
plotname <- paste0("/oceano/gmeteo/WORK/lisette/Trabajo/Writing/paper/figures/comparegraphsdistances.pdf")
pdf(plotname, width = 14, height = 8)
op <- par(no.readonly = TRUE)
lay <- rbind(c(1,0,0),
             c(2,3,0),
...
load("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/pcstable/pc_1_eBIC_g5.rda")
# best of pcstable
...
load("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/permutations.rda")
load("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/pcstable/pc_1_eBIC_g5.rda")
load("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/mmhcsizegraphs/tabu_simple_1600_2.rda")
load("/oceano/gmeteo/WORK/lisette/Trabajo/R_practice/Data/Struct_learn/mmhcsizegraphs/tabu_mmhc_simple2.rda")
# best of pcstable
plot_long_distances(pcstable2_withnet_1$networks$pcstable_10d_g0.1, data.dag = TimeCoordsAnom_from_Grid_std(tas_ncep_10d),minimdist = 5000, smallcol = rgb(0,0,255,alpha = 100, maxColorValue = 255), perm = permutations[[1]],title = FALSE)
title(mar = c(0,0,0,0), main = paste0("|E| = ",narcs(pc_1_eBIC_g5)))
# # pcstablbest and best of mmhc
...
######################################################################
# Investigating reason of failure GS and PC. 
######################################################################
mmhc_1_eBIC_g0$networks$mmhc_1_eBIC_g0
plot_long_distances(mmhc_5_eBIC_g0$networks$mmhc_5_eBIC_g0, data.dag = TimeCoordsAnom_from_Grid_std(tas_ncep_10d),minimdist = 5000, smallcol = rgb(0,0,255,alpha = 100, maxColorValue = 255), perm = permutations[[5]],title = TRUE)
title(mar = c(0,0,0,0), main = paste0("|E| = ",narcs(mmhc_1_eBIC_g0$networks$mmhc_10d_g0)))

plot_long_distances(gs_withnet_2$networks$gs_10d_g0.1, data.dag = TimeCoordsAnom_from_Grid_std(tas_ncep_10d),minimdist = 5000, smallcol = rgb(0,0,255,alpha = 100, maxColorValue = 255), perm = permutations[[2]],title = TRUE)
title(mar = c(0,0,0,0), main = paste0("|E| = ",narcs(gs_withnet_1$networks$gs_10d_g0.1)))

acyclic(gs_withnet_1$networks$gs_10d_g0.1, directed = FALSE, debug = TRUE)
acyclic(pcstable2_withnet_1$networks$pcstable_10d_g0.25, directed = FALSE)
cextend(pcstable2_withnet_1$networks$pcstable_10d_g9)

undirected.arcs(gs_withnet_1$networks$gs_10d_g0.1)
########################################################################
# Use plot undirected distances. 
##########################################################################
plotname <- "/oceano/gmeteo/WORK/lisette/Trabajo/PGMext/paper/figures/gsundirected_new.pdf"
pdf(plotname)
...
plot_undirected_distances(gs_withnet_5$networks$gs_10d_g0.1,data.dag = TimeCoordsAnom_from_Grid_std(tas_ncep_10d),minimdist = 5000, smallcol = rgb(0,0,255,alpha = 100, maxColorValue = 255), perm = permutations[[5]],title = TRUE, arrowmode = TRUE)
dev.off()



plot_undirected_distances(pcstable2_withnet_4$networks$pcstable_10d_g0.1,data.dag = TimeCoordsAnom_from_Grid_std(tas_ncep_10d),minimdist = 5000, smallcol = rgb(0,0,255,alpha = 100, maxColorValue = 255), perm = permutations[[1]],title = TRUE)
title(mar = c(0,0,0,0), main = paste0("|E| = ",narcs(pcstable2_withnet_3$networks$pcstable_10d_g0.1)))


vstructs(pcstable2_withnet_1$networks$pcstable_10d_g9,moral = FALSE)
vstructs(pcstable2_withnet_1$networks$pstable_10d_g9, moral = FALSE)
bnlearn::plot(pcstable_withnet_1$networks$gs_10d_g9)
acyclic(gs_withnet_2$networks$gs_10d_g9,directed = TRUE)

gs_withnet_1$networkdata
pcstable2_withnet_1$networkdata
narcs()
gs()
...
gs_withnet_1$networkdata
pcstable2_withnet_1$networkdata
gs_withnet_2$networkdata
gs_withnet_3$networkdata
gs_withnet_4$networkdata
gs_withnet_5$networkdata
gs_1$networkdata
gs_2$networkdata

graphGraph
is_chordal(igraph.from.graphNEL(as.graphNEL(gs_withnet_1$networks$gs_10d_g5)), alpha = NULL, alpham1 = NULL, fillin = FALSE,
           newgraph = FALSE)
pcstable2_withnet_1$networks$pcstable_10d_g5
gs_withnet_1$networks$gs_10d_g5
...
ejes2tabuBIC <- lapply(list(tabu_1_eBIC,tabu_2_eBIC,tabu_3_eBIC,tabu_4_eBIC,tabu_5_eBIC),ejes_gamma_log)
ejes2hcBIC <- lapply(list(hc_1_eBIC, hc_2_eBIC, hc_3_eBIC,hc_4_eBIC,hc_5_eBIC),ejes_gamma_log)
ejes2mmhcBIC <- lapply(list(mmhc_1_eBIC,mmhc_2_eBIC,mmhc_3_eBIC,mmhc_4_eBIC,mmhc_5_eBIC), ejes_gamma_log)
ejes2h2pcBIC <- lapply(list(h2pc_1_eBIC,h2pc_2_eBIC,h2pc_3_eBIC,h2pc_4_eBIC,h2pc_5_eBIC), ejes_gamma_log)
ejes2pcstable <- lapply(list(pcstable2_1, pcstable2_2, pcstable2_3,pcstable2_4,pcstable2_5),ejes_gamma_log)
ejes2gs <- lapply(list(gs_1, gs_2, gs_3, gs_4, gs_5), ejes_gamma_edges)


par(xpd=F)
plot(ejestabuBIC[[5]][[2]], ejes2tabuBIC[[5]][[2]], type = "p",
     #lwd = 1,
     # pch = 23,
     col = rgb(255,0,0, alpha = 255, maxColorValue = 255),
     main = "(c) Size",
...
points(ejestabuBIC[[2]][[2]], ejes2tabuBIC[[2]][[2]], col = rgb(255,0,0, alpha = 125, maxColorValue = 255))
points(ejestabuBIC[[3]][[2]], ejes2tabuBIC[[3]][[2]], col = rgb(255,0,0, alpha = 125, maxColorValue = 255))
points(ejestabuBIC[[4]][[2]], ejes2tabuBIC[[4]][[2]], col = rgb(255,0,0, alpha = 125, maxColorValue = 255))
points(ejestabuBIC[[1]][[2]], ejes2tabuBIC[[1]][[2]], col = rgb(255,0,0, alpha = 125, maxColorValue = 255))

points(ejeshcBIC[[5]][[2]], ejes2hcBIC[[5]][[2]], col = rgb(255,165,0, alpha = 255, maxColorValue = 255))
points(ejeshcBIC[[2]][[2]], ejes2hcBIC[[2]][[2]], col = rgb(255,165,0, alpha = 255, maxColorValue = 255))
points(ejeshcBIC[[3]][[2]], ejes2hcBIC[[3]][[2]], col = rgb(255,165,0, alpha = 255, maxColorValue = 255))
points(ejeshcBIC[[4]][[2]], ejes2hcBIC[[4]][[2]], col = rgb(255,165,0, alpha = 255, maxColorValue = 255))
points(ejeshcBIC[[1]][[2]], ejes2hcBIC[[1]][[2]], col = rgb(255,165,0, alpha = 255, maxColorValue = 255))

points(ejesmmhcBIC[[5]][[2]], ejes2mmhcBIC[[5]][[2]], col = rgb(0,255,0, alpha = 255, maxColorValue = 255))
points(ejesmmhcBIC[[2]][[2]], ejes2mmhcBIC[[2]][[2]], col = rgb(0,255,0, alpha = 255, maxColorValue = 255))
points(ejesmmhcBIC[[3]][[2]], ejes2mmhcBIC[[3]][[2]], col = rgb(0,255,0, alpha = 255, maxColorValue = 255))
points(ejesmmhcBIC[[4]][[2]], ejes2mmhcBIC[[4]][[2]], col = rgb(0,255,0, alpha = 255, maxColorValue = 255))
points(ejesmmhcBIC[[1]][[2]], ejes2mmhcBIC[[1]][[2]], col = rgb(0,255,0, alpha = 255, maxColorValue = 255))

points(ejesh2pcBIC[[5]][[2]], ejes2h2pcBIC[[5]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[2]][[2]], ejes2h2pcBIC[[2]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[3]][[2]], ejes2h2pcBIC[[3]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[4]][[2]], ejes2h2pcBIC[[4]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))
points(ejesh2pcBIC[[1]][[2]], ejes2h2pcBIC[[1]][[2]], col = rgb(0,100,0, alpha = 255, maxColorValue = 255))


col2rgb("navyblue")
points(ejespcstable[[5]][[2]], ejes2pcstable[[5]][[2]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[2]][[2]], ejes2pcstable[[2]][[2]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[3]][[2]], ejes2pcstable[[3]][[2]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[4]][[2]], ejes2pcstable[[4]][[2]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))
points(ejespcstable[[1]][[2]], ejes2pcstable[[1]][[2]], col = rgb(0,0,128, alpha = 125, maxColorValue = 255))

points(ejesgs[[5]][[2]], ejes2gs[[5]][[2]], col = rgb(0,0,255, alpha = 255, maxColorValue = 255))
points(ejesgs[[2]][[2]], ejes2gs[[2]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[3]][[2]], ejes2gs[[3]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[4]][[2]], ejes2gs[[4]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
points(ejesgs[[1]][[2]], ejes2gs[[1]][[2]], col = rgb(0,0,255, alpha = 125, maxColorValue = 255))
